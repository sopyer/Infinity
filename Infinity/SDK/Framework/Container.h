namespace UI
{
	class Container: public Actor
	{
		...
		public:
			...
			Container& add(Actor* child) {mChilds.push_back(child); /*mChild.setParent(this);*/ queueRelayout(); return *this;}

			Container& remove(Actor& child) {return remove(&child);}
			Container& remove(Actor* child) {std::remove(mChilds.begin(), mChilds.end(), child); /*mChild.setParent(0);*/ return *this;}
