...
__forceinline v128 vi_set_xxxx_i32(int x)
{
    return _mm_castsi128_ps(_mm_set1_epi32(x));
}

__forceinline v128 vi_set_x000_i32(int x)
{
    return _mm_castsi128_ps(_mm_cvtsi32_si128(x));
}

__forceinline v128 vi_load(void* m128)
{
	return _mm_load_ps((float*)m128);
}

__forceinline v128 vi_loadu(void* m128)
{
	return _mm_loadu_ps((float*)m128);
}

__forceinline v128 vi_load_zero()
{
	return _mm_setzero_ps();
}

__forceinline v128 vi_load_one()
{
	return _mm_set_ps1(1.0f);
}

__forceinline void vi_store(void* m128, v128 val)
{
    _mm_store_ps((float*)m128, val);
}

__forceinline void vi_storeu(void* m128, v128 val)
{
    _mm_storeu_ps((float*)m128, val);
}

__forceinline void vi_storex(void* m32, v128 val)
{
    _mm_store_ss((float*)m32, val);
}
...
{
	return _mm_xor_ps(a, vi_set_xxxx_i32(0x80000000));
}

__forceinline v128 vi_abs(v128 a)
{
	return _mm_and_ps(a, vi_set_xxxx_i32(0x7FFFFFFF));
}

__forceinline v128 vi_sign(v128 a)
{
	v128 one = vi_and(vi_cmp_ne(a, vi_load_zero()), vi_set_xxxx_i32(0x3F800000));
	v128 sign = vi_and(a, vi_set_xxxx_i32(0x80000000));
...
{
	return vi_clamp(value, vi_load_zero(), vi_load_one());
}
