...
namespace glm
{
    ...
    template <typename T> 
    inline _xmat4<T> fastInverseGTX(const _xmat4<T>& m)
    {
		_xmat4<T> Result;
		Result[3] = _xvec4<T>(0, 0, 0, 1);
		Result = glm::transposeGTX(Result);
		_xvec4<T> Translation = Result * -m[3];
...
/*
    template <typename T> 
    inline _xmat2<T> affineInverseGTX(const _xmat2<T>& m)
    {
        T Determinant = m[0][0] * m[1][1] - m[1][0] * m[0][1];
    ...
    template <typename T> 
    inline _xmat3<T> affineInverseGTX(const _xmat3<T>& m)
    {
        _xmat3<T> Result;
        return Result;
    }
    ...
    template <typename T>
    inline _xmat4<T> affineInverseGTX(const _xmat4<T>& m)
    {
        #define ACCUMULATE() \
