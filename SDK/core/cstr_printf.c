...
getexponent(double value)
{
	...
	/*
	 * We check for DBL_MAX_10_EXP >= exponent >= DBL_MIN_10_EXP in
	 ...
	 */
	while (tmp < 1.0 && tmp > 0.0 && --exponent >= DBL_MIN_10_EXP)
		tmp *= 10;
	while (tmp >= 10.0 && ++exponent <= DBL_MAX_10_EXP)
		tmp /= 10;
...
cast(double value)
{
	 ...
	 * represented exactly as an double value (but is less than LDBL_MAX),
	 * it may be increased to the nearest higher representable value for the
	 * comparison (cf. C99: 6.3.1.4, 2).  It might then equal the double
...
static void
fmtflt(char *str, size_t *len, size_t size, double fvalue, int width,
...
int
cstr_vprintf(char *str, size_t size, const char *format, va_list args)
{
	double fvalue;
	intmax_t value;
...
int
cstr_printf(char *str, size_t size, const char *format, ...)
{
	va_list ap;
	int len;

	va_start(ap, format);
	len = cstr_vprintf(str, size, format, ap);
	va_end(ap);
	return len;
}
